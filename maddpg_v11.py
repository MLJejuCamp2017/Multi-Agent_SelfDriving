...
def euclidean(x, y):
    ...
    return dist
#%%
maxLength = euclidean(np.array([-1,-1]), np.array([1, 1]))
#%%
def makeMultiState(s_t, my_point):
    colors = s_t[:,0][:,None]
    points = s_t[:,1:3]
    dists = np.zeros(s_t.shape[0])
    for i in xrange(s_t.shape[0]):
        dists[i] = np.abs(euclidean(points[i], my_point)-maxLength)/maxLength
    lastState = np.hstack((colors, dists[:,None], points))
    return lastState
...
def addToBufferMulti3(buff, s_t, a_t, r_t, s_t1, a_t1, done, numCars=2):
    ...
    # for critic states
    eucl_state = np.zeros((numCars, 4*numCars))
    eucl_new_state = np.zeros((numCars, 4*numCars))
    for i in xrange(s_t.shape[0]):
        eucl_state[i] = makeMultiState(s_t, s_t[i,1:3]).flatten()
        eucl_new_state[i] = makeMultiState(s_t1, s_t1[i, 1:3]).flatten()
    
    s_tt = np.zeros((numCars, eucl_state.shape[1]+a_t.shape[1]*(numCars)))
    s_tt1 = np.zeros((numCars, eucl_state.shape[1]+a_t.shape[1]*(numCars)))
    for i in xrange(numCars):
        s_tt[i] = np.hstack((eucl_state[i], a_t.flatten()))
        s_tt1[i] = np.hstack((eucl_new_state[i], a_t1.flatten()))
...
#%%
    ...
    m_state_dim = 4*numCars # color, inverse euclidean
    m_action_dim = action_dim*numCars
...
if __name__ == "__main__":
    ...
    parser.add_option("-n", "--num", dest="n_cars", type="int", default=16,
        help="Number of Cars")
    parser.add_option("-s", "--save", dest="save", default="Dropbox/06.MLJeju/MADDPG2/results/ma11_last",
