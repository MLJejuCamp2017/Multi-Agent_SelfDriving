...
def makeMultiState(s_t, my_point):
    ...
    lastState = np.hstack((colors, dists[:,None]))
    return lastState
...
def addToBufferMulti3(buff, s_t, a_t, r_t, s_t1, a_t1, done, numCars=2):
    ...
    # for critic states
    eucl_state = np.zeros((numCars, 2*numCars))
    eucl_new_state = np.zeros((numCars, 2*numCars))
    for i in xrange(s_t.shape[0]):
...
#%%
    ...
    m_state_dim = 2*numCars # color, inverse euclidean
...
if __name__ == "__main__":
    ...
    parser.add_option("-n", "--num", dest="n_cars", type="int", default=16,
        help="Number of Cars")
    parser.add_option("-s", "--save", dest="save", default="Dropbox/06.MLJeju/MADDPG2/results/ma10",
